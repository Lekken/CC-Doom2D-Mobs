...
-- GFX (for debugging)
local sprite = loadgfx("Doom2D/environment/water30x30.png")
setmidhandle(sprite)
...
if d2d.debug ~= nil and d2d.debug.DEBUGGING_LIQUIDS_FOR_EDITOR then
	d2d.water30x30.id = addobject("d2d.water30x30", sprite)
else
	d2d.water30x30.id = addobject("d2d.water30x30", 0)
end

function d2d.water30x30.setup(id, parameter)
	if objects == nil then objects = {} end
	objects[id] = {}
end

function d2d.water30x30.update(id, x, y)
...
function d2d.water30x30.draw(id, x, y)
...
function d2d.water30x30.damage(id, damage)
