--[[
Doom 2D Enemies: Chaingun Dude
Script by Lekken
--]]

--------------------------------------------------------------------------------
-- Object: Chaingun Dude
--------------------------------------------------------------------------------
...
animations.death.duration = d2d.default_fps / 1.5
...
-- Timers constants
local max_delay_before_spawn_projectile = 5
...
for i = 1, animations.move.frames do
	sprites.move[i] = loadgfx("Doom2D/mobs/ChaingunDude/CGUN_GO_"..i..".png")
	setmidhandle(sprites.move[i])
end
-- Attack
sprites.attack = {}
for i = 1, animations.attack.frames do
	sprites.attack[i] = loadgfx("Doom2D/mobs/ChaingunDude/CGUN_ATTACK_"..i..".png")
	setmidhandle(sprites.attack[i])
end
-- Death
sprites.death = {}
for i = 1, animations.death.frames do
	sprites.death[i] = loadgfx("Doom2D/mobs/ChaingunDude/CGUN_DIE_"..i..".png")
...
local sounds = {}
-- Shot
sounds.shot = loadsfx("Doom2D/effects/FIRECGUN.ogg")
-- Sight
sounds.sight = {}
for i = 1, 3 do
...
-- Creating
d2d.mob.cgun.id = addobject("d2d.mob.cgun", sprites.move[1])

function d2d.mob.cgun.setup(id, parameter)
	...
	d2d.mob.cgun.reset_animation(id)
end

function d2d.mob.cgun.update(id, x, y)
	-- Processing
	objects[id].x = x
	objects[id].y = y
	objects[id].elevation_relative_to_player = d2d.mob.cgun.calculate_elevation(id)
	--
	...
	d2d.mob.cgun.fire_damage(id)	
	d2d.mob.cgun.animation(id)
	d2d.mob.cgun.actions(id)
	
	-- Debug
	if d2d.debug ~= nil and d2d.debug.DEBUG_LOG then
		d2d.debug.log("Chaingun Dude", objects[id])
	end
end

function d2d.mob.cgun.draw(id, x, y)
...
function d2d.mob.cgun.damage(id, damage)
	...
	if objects[id].health <= 0 then
		d2d.mob.cgun.change_state_to(id, d2d.mob_states.DEAD)
...
function d2d.mob.cgun.fire_damage(id)	
...
function d2d.mob.cgun.animation(id)
	...
	if objects[id].state == d2d.mob_states.DEAD then
		d2d.mob.cgun.animation_death(id)
		return
	end
	-- Attack
	if objects[id].state == d2d.mob_states.ATTACK then
		d2d.mob.cgun.animation_attack(id)
	-- Move
	else
		d2d.mob.cgun.animation_move(id)
	end
end

function d2d.mob.cgun.animation_move(id)
	d2d.mob.cgun.play_animation(id, animations.move)
	objects[id].current_gfx = sprites.move[objects[id].current_animation_frame]
end

function d2d.mob.cgun.animation_attack(id)
	d2d.mob.cgun.play_animation(id, animations.attack)
	objects[id].current_gfx = sprites.attack[objects[id].current_animation_frame]
end

function d2d.mob.cgun.animation_death(id)
	d2d.mob.cgun.play_animation(id, animations.death, true)
	objects[id].current_gfx = sprites.death[objects[id].current_animation_frame]
end

function d2d.mob.cgun.play_animation(id, animation, once)
...
function d2d.mob.cgun.reset_animation(id)
...
function d2d.mob.cgun.calculate_direction(id)
...
function d2d.mob.cgun.calculate_elevation(id)
...
function d2d.mob.cgun.change_state_to(id, state)
	objects[id].prev_state = objects[id].state
...
function d2d.mob.cgun.actions(id)
	d2d.mob.cgun.gravity(id)
	
	if objects[id].state == d2d.mob_states.DEAD then
		d2d.mob.cgun.death(id)
		return
	end
		
	if objects[id].state == d2d.mob_states.ATTACK then
		d2d.mob.cgun.attack(id)
	else
		d2d.mob.cgun.move(id)
	end
	
	d2d.mob.cgun.look_for(id)
end

function d2d.mob.cgun.process_collisions(id)
...
function d2d.mob.cgun.gravity(id)
...
function d2d.mob.cgun.move(id)	
	if objects[id].is_seeing then
		objects[id].direction = d2d.mob.cgun.calculate_direction(id)
		d2d.mob.cgun.chase(id)
	else
		d2d.mob.cgun.waiting(id)
	end
	
	d2d.mob.cgun.process_collisions(id)
	
	objectposition(id, objects[id].x, objects[id].y)
end

function d2d.mob.cgun.chase(id)
...
function d2d.mob.cgun.waiting(id)
...
function d2d.mob.cgun.look_for(id)
	-- Can see the player?
	if d2d.mob.cgun.can_see(id) then
		objects[id].is_seeing = true
		...
		if objects[id].distance <= distance_attack then	
			d2d.mob.cgun.change_state_to(id, d2d.mob_states.ATTACK)
		else
			d2d.mob.cgun.change_state_to(id, d2d.mob_states.MOVE)
			objects[id].timers.delay_before_spawn_projectile = max_delay_before_spawn_projectile
		end
	else
		objects[id].is_seeing = false
...
function d2d.mob.cgun.can_see(id)
...
function d2d.mob.cgun.death(id)
