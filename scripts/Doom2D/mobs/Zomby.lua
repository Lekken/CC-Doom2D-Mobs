...
for i = 1, animations.move.frames do
	sprites.move[i] = loadgfx("Doom2D/mobs/Zomby/ZOMBY_GO_"..i..".png")
	setmidhandle(sprites.move[i])
end
-- Attack
sprites.attack = {}
for i = 1, animations.attack.frames do
	sprites.attack[i] = loadgfx("Doom2D/mobs/Zomby/ZOMBY_ATTACK_"..i..".png")
	setmidhandle(sprites.attack[i])
end
-- Death
sprites.death = {}
for i = 1, animations.death.frames do
	sprites.death[i] = loadgfx("Doom2D/mobs/Zomby/ZOMBY_DIE_"..i..".png")
...
-- Creating
d2d.mob.zomby.id = addobject("d2d.mob.zomby", sprites.move[1])

function d2d.mob.zomby.setup(id, parameter)
	...
	d2d.mob.zomby.reset_animation(id)
end

function d2d.mob.zomby.update(id, x, y)
	-- Processing
	objects[id].x = x
	objects[id].y = y
	objects[id].elevation_relative_to_player = d2d.mob.zomby.calculate_elevation(id)
	--
	...
	d2d.mob.zomby.fire_damage(id)
	d2d.mob.zomby.animation(id)
	d2d.mob.zomby.actions(id)
	
	-- Debug
	if d2d.debug ~= nil and d2d.debug.DEBUG_LOG then
		d2d.debug.log("Zomby", objects[id])
	end
end

function d2d.mob.zomby.draw(id, x, y)
...
function d2d.mob.zomby.damage(id, damage)
	...
	if objects[id].health <= 0 then
		d2d.mob.zomby.change_state_to(id, d2d.mob_states.DEAD)
...
function d2d.mob.zomby.fire_damage(id)	
...
function d2d.mob.zomby.animation(id)
	...
	if objects[id].state == d2d.mob_states.DEAD then
		d2d.mob.zomby.animation_death(id)
		return
	end
	-- Attack
	if objects[id].state == d2d.mob_states.ATTACK then
		d2d.mob.zomby.animation_attack(id)
	-- Move
	else
		d2d.mob.zomby.animation_move(id)
	end
end

function d2d.mob.zomby.animation_move(id)
	d2d.mob.zomby.play_animation(id, animations.move)
	objects[id].current_gfx = sprites.move[objects[id].current_animation_frame]
end

function d2d.mob.zomby.animation_attack(id)
	if not objects[id].delay_attack then
		d2d.mob.zomby.play_animation(id, animations.attack)
	else
...
function d2d.mob.zomby.animation_death(id)
	d2d.mob.zomby.play_animation(id, animations.death, true)
	objects[id].current_gfx = sprites.death[objects[id].current_animation_frame]
end

function d2d.mob.zomby.play_animation(id, animation, once)
...
function d2d.mob.zomby.reset_animation(id)
...
function d2d.mob.zomby.calculate_direction(id)
...
function d2d.mob.zomby.calculate_elevation(id)
...
function d2d.mob.zomby.change_state_to(id, state)
	objects[id].prev_state = objects[id].state
...
function d2d.mob.zomby.actions(id)
	d2d.mob.zomby.gravity(id)
	
	if objects[id].state == d2d.mob_states.DEAD then
		d2d.mob.zomby.death(id)
		return
	end
		
	if objects[id].state == d2d.mob_states.ATTACK then
		d2d.mob.zomby.attack(id)
	else
		d2d.mob.zomby.move(id)
	end
	
	d2d.mob.zomby.look_for(id)
end

function d2d.mob.zomby.process_collisions(id)
...
function d2d.mob.zomby.gravity(id)
...
function d2d.mob.zomby.move(id)	
	if objects[id].is_seeing then
		objects[id].direction = d2d.mob.zomby.calculate_direction(id)
		d2d.mob.zomby.chase(id)
	else
		d2d.mob.zomby.waiting(id)
	end
	d2d.mob.zomby.process_collisions(id)
	
	objectposition(id, objects[id].x, objects[id].y)
end

function d2d.mob.zomby.chase(id)
...
function d2d.mob.zomby.waiting(id)
...
function d2d.mob.zomby.look_for(id)
	-- Can see the player?
	if d2d.mob.zomby.can_see(id) then
		objects[id].is_seeing = true
		...
		if objects[id].distance <= distance_attack then
			...
			if objects[id].timers.delay_between_attacks < 0 then
				d2d.mob.zomby.change_state_to(id, d2d.mob_states.ATTACK)
			end
		else
			d2d.mob.zomby.change_state_to(id, d2d.mob_states.MOVE)
			objects[id].timers.delay_between_attacks = 0
			objects[id].timers.delay_before_spawn_projectile = max_delay_before_spawn_projectile
		end
	else
...
function d2d.mob.zomby.can_see(id)
...
function d2d.mob.zomby.attack(id)
	...
	if not objects[id].delay_attack then
		...
		then
			...
			d2d.projectiles.bullet.create(objects[id].x + objects[id].direction * sprites.height / 3,
										  objects[id].y - sprites.height / 10,
...
function d2d.mob.zomby.death(id)
