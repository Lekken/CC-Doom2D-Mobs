...
for i = 1, animations.move.frames do
	sprites.move[i] = loadgfx("Doom2D/mobs/Human/MAN_GO_"..i..".png")
	setmidhandle(sprites.move[i])
end
-- Attack
sprites.attack = {}
for i = 1, animations.attack.frames do
	sprites.attack[i] = loadgfx("Doom2D/mobs/Human/MAN_ATTACK_"..i..".png")
	setmidhandle(sprites.attack[i])
end
-- Death
sprites.death = {}
for i = 1, animations.death.frames do
	sprites.death[i] = loadgfx("Doom2D/mobs/Human/MAN_DIE_"..i..".png")
	setmidhandle(sprites.death[i])
end

-- SFX
local sounds = {}
sounds.shot = loadsfx("Doom2D/effects/FIREPISTOL.ogg")
sounds.sight = loadsfx("ALERT_3.ogg")
sounds.death = loadsfx("Doom2D/mobs/Humanoids/MAN_DIE.ogg")
...
-- Creating
d2d.mob.fman.id = addobject("d2d.mob.fman", sprites.move[1])

function d2d.mob.fman.setup(id, parameter)
	...
	objects[id].timer_delay_between_attacks = 0
	objects[id].timers.firedamage_tick = d2d.fire.max_timer 
	objects[id].timer_disappear_after_death = d2d.default_fps / 10
	...
	d2d.mob.fman.animation_reset(id)
end

function d2d.mob.fman.update(id, x, y)
	-- Processing
	objects[id].x = x
	objects[id].y = y
	objects[id].elevation_relative_to_player = d2d.mob.fman.calculate_elevation(id)
	--
	...
	d2d.mob.fman.fire_damage(id)
	d2d.mob.fman.animation(id)
	d2d.mob.fman.actions(id)
	
	-- Debug
	if d2d.debug ~= nil and d2d.debug.DEBUG_LOG then
		d2d.debug.log("Former Man", objects[id])
	end
end

function d2d.mob.fman.draw(id, x, y)
...
function d2d.mob.fman.damage(id, damage)
	...
	if objects[id].health <= 0 then
		d2d.mob.fman.change_state_to(id, d2d.mob_states.DEAD)
...
function d2d.mob.fman.fire_damage(id)	
...
-- Animations
-----------------------
function d2d.mob.fman.animation(id)
	...
	if objects[id].state == d2d.mob_states.DEAD then
		d2d.mob.fman.animation_death(id)
		return
	end
	-- Attack
	if objects[id].state == d2d.mob_states.ATTACK then
		d2d.mob.fman.animation_attack(id)
	-- Move
	else
		d2d.mob.fman.animation_move(id)
	end
end

function d2d.mob.fman.animation_move(id)
	d2d.mob.fman.play_animation(id, animations.move)
	objects[id].current_gfx = sprites.move[objects[id].current_animation_frame]
end

function d2d.mob.fman.animation_attack(id)
	if not objects[id].delay_attack then
		d2d.mob.fman.play_animation(id, animations.attack)
	else
...
function d2d.mob.fman.animation_death(id)
	d2d.mob.fman.play_animation(id, animations.death, true)
	objects[id].current_gfx = sprites.death[objects[id].current_animation_frame]
end

function d2d.mob.fman.play_animation(id, animation, once)
...
function d2d.mob.fman.animation_reset(id)
...
function d2d.mob.fman.calculate_direction(id)
...
function d2d.mob.fman.calculate_elevation(id)
...
function d2d.mob.fman.change_state_to(id, state)
	objects[id].prev_state = objects[id].state
...
function d2d.mob.fman.actions(id)
	d2d.mob.fman.gravity(id)
	
	if objects[id].state == d2d.mob_states.DEAD then
		d2d.mob.fman.death(id)
		return
	end
		
	if objects[id].state == d2d.mob_states.ATTACK then
		d2d.mob.fman.attack(id)
	else
		d2d.mob.fman.move(id)
	end
	
	d2d.mob.fman.look_for(id)
end

function d2d.mob.fman.process_collisions(id)
...
function d2d.mob.fman.gravity(id)
...
function d2d.mob.fman.move(id)	
	if objects[id].is_seeing then
		objects[id].direction = d2d.mob.fman.calculate_direction(id)
		d2d.mob.fman.chase(id)
	else
		d2d.mob.fman.waiting(id)
	end
	
	d2d.mob.fman.process_collisions(id)
	
	objectposition(id, objects[id].x, objects[id].y)
end

function d2d.mob.fman.chase(id)
...
function d2d.mob.fman.waiting(id)
...
function d2d.mob.fman.look_for(id)
	-- Can see the player?
	if d2d.mob.fman.can_see(id) then
		objects[id].is_seeing = true
		...
		if objects[id].distance <= distance_attack then
			objects[id].timer_delay_between_attacks = objects[id].timer_delay_between_attacks - 1
			if objects[id].timer_delay_between_attacks < 0 then
				d2d.mob.fman.change_state_to(id, d2d.mob_states.ATTACK)
			end
		else
			d2d.mob.fman.change_state_to(id, d2d.mob_states.MOVE)
			objects[id].timer_delay_between_attacks = 0
			objects[id].timers.delay_before_spawn_projectile = max_delay_before_spawn_projectile
		end
	else
		objects[id].is_seeing = false
		d2d.mob.fman.change_state_to(id, d2d.mob_states.MOVE)
		objects[id].timer_delay_between_attacks = max_delay_between_attacks
		objects[id].oneshot_playsound_sigh = false
	end
end

function d2d.mob.fman.can_see(id)
...
function d2d.mob.fman.attack(id)
...
function d2d.mob.fman.death(id)
	if objects[id].current_animation_frame == animations.death.frames then
		objects[id].timer_disappear_after_death = objects[id].timer_disappear_after_death - 1
		if objects[id].timer_disappear_after_death <= 0 then
			removeobject(id)
		end
	end
end